...
class MusicUpgradeGUI:
    ...
    async def match_files_async(self):
       ...
       for index, music_file in enumerate(self.music_files):
           ...
           try:
               ...
               existing_match = self.matched_songs[index]
               ...
               # 更新表格显示 - 第二列只显示音乐匹配信息
               current_match = self.matched_songs[index] if self.matched_songs[index] else matched_song
               ...
               if existing_match and existing_match.get('id'):
                   ...
                   # 显示原有的匹配信息
                   display_text = f"{existing_match.get('name', '未知')} - {existing_match.get('artist', ['未知'])[0] if isinstance(existing_match.get('artist'), list) else existing_match.get('artist', '未知')}"
                   self.root.after(0, lambda idx=index, text=display_text: self.update_table_item(idx, text, MusicStatus.AUTO_MATCHED.value))
               else:
    ...
    def ignore_item(self, item):
        ...
        self.tree.item(item, values=(current_values[0], "", MusicStatus.IGNORED.value))  # 忽略时第二列显示空白
    ...
    async def _rematch_single_file_async(self, index, search_keyword):
       ...
       try:
           ...
           if existing_match and existing_match.get('id'):
               # 如果已有匹配结果且匹配成功，则跳过本次匹配，保持原有匹配信息
               logger.info(f"文件 {self.music_files[index].name} 已有匹配结果，跳过本次匹配")
               # 更新状态，但保持原有匹配信息
               if self.status_manager:
                   old_status = self.status_manager.get_status(index)
                   self.status_manager.set_status(index, MusicStatus.AUTO_MATCHED)
           ...
           if existing_match and existing_match.get('id'):
               # 保持原有匹配结果
               logger.info(f"文件 {self.music_files[index].name} 已有匹配结果，保持原有匹配信息")
               # 更新状态，但保持原有匹配信息
               if self.status_manager:
                   old_status = self.status_manager.get_status(index)
                   self.status_manager.set_status(index, MusicStatus.AUTO_MATCHED)
        ...
        try:
            ...
            if matched_song and matched_song.get('id'):
                ...
                if download_path:
                    ...
                    # 下载失败时，第二列保持不变，只更新状态
                    self.root.after(0, lambda idx=index: self.update_table_item(idx, None, MusicStatus.DOWNLOAD_FAIL.value))
            ...
            if can_download and matched_song and matched_song.get('id'):
                try:
                    ...
                    if download_path:
                        ...
                        # 下载完成后，第二列保持不变，只更新状态
                        self.root.after(0, lambda idx=i: self.update_table_item(idx, None, MusicStatus.AUTO_DOWNLOAD_COMPLETE.value))
                    else:
                        fail_count += 1
                        ...
                        if self.status_manager:
                            ...
                            logger.debug(f"状态转换: {old_status.value if old_status else 'None'} -> {MusicStatus.DOWNLOAD_FAIL.value}")
                        # 下载失败时，第二列保持不变，只更新状态
                        self.root.after(0, lambda idx=i: self.update_table_item(idx, None, MusicStatus.DOWNLOAD_FAIL.value))
                except Exception as e:
                    fail_count += 1
